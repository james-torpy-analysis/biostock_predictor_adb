...
max_days_in_past = 2 # Min value must be within this many days in the past from current date
subset_tickers = 10 # Number of tickers to download. If None, all tickers will be downloaded
...
def predict_anomalies(ticker, df, cont_val, ticker_count, total_ticker_no, pyspark = True):
    ...
    '''
    try:
        print(f'Predicting anomalies for {ticker} ({ticker_count}/{total_ticker_no} tickers)...')
        df['date'] = df.index
        ...
        total_no = len(df)
        print(f'{no_predicted} anomalies predicted for {ticker} out of {total_no} dates')
        ...
        if len(df) == 0:
            print(f'No anomalies predicted for {ticker}')
            return None
        else :
            no_anomalies = len(df)
            if pyspark:
        ...
        print(f'Failed to predict anomalies for {ticker}')
        return None
...
def find_crashes(ticker, anomaly_df, orig_df, perc_drop_req, perc_drop_time, max_days_in_past):
    print(f'Finding crashes for {ticker}...')
    ...
    try:
        ...
        crashes = crashes.select('anomaly_adj_close', 'anomaly_date', 'preanomaly_date', 'preanomaly_close', \
        'preanomaly_close_diff', 'preanomaly_close_percent_diff', 'period_end_date', 'Volume', 'VolumeClose')
        ...
        # Add ticker
        crash_stats = crash_stats.withColumn('ticker', lit(ticker))

        # Select and rename columns
        crash_stats = crash_stats.select('ticker', 'anomaly_date', 'anomaly_adj_close', 'Volume', 'VolumeClose', 'preanomaly_date', \
            'preanomaly_close', 'preanomaly_close_diff', 'preanomaly_close_percent_diff', 'period_end_date', 'min_max_diff', 'min_max_percent_diff')
        new_names = ['ticker', 'crash_date', 'crash_close', 'crash_volume', 'crash_volume_close', 'precrash_date', \
        ...
        # Filter for crashes where period_percent_max_diff <= -perc_drop_req and min_value_date within max_days_in_past days
        crash_stats = crash_stats.filter((crash_stats['period_percent_max_diff'] <= -perc_drop_req) & (crash_stats['crash']) & \
            crash_stats['min_value_date'].between(F.date_sub(F.col('date'), max_days_in_past), F.col('date'])
        crash_stats = crash_stats.drop('crash')
        
        return crash_stats
    
    except Exception as e:
        print(f'Error identifying crashes for {ticker}: {e}')
        return None
...
def plot_crashes(df, crashes, ticker):
    ...
    ax.set_title(f"{ticker} Crash Detection")
    ax.legend()
    plt.tight_layout()
...
def main():
    ...
    # Remove tickers with NaN
    nasdaq_data = {key: value for key, value in nasdaq_data.items() if not value.isna().all()['Adj Close']}

    # Step 2: Anomaly Detection
    anomaly_data = {}
    for i, (ticker, df) in enumerate(nasdaq_data.items()):
        anomalies = predict_anomalies(ticker, df, cont_val, ticker_count = i+1, total_ticker_no = len(nasdaq_data))
        if anomalies:
            if not anomalies.isEmpty():
                anomaly_data[ticker] = anomalies

    total_count = len(nasdaq_data)
    anomaly_count = len(anomaly_data)
    none_count = total_count - anomaly_count
    print(f'At least one anomaly was detected in {anomaly_count} out of {total_count} tickers')
    ...
    crash_results = {}
    for ticker in anomaly_data.keys():
        crashes = find_crashes(ticker, anomaly_data[ticker], nasdaq_data[ticker], \
    ...
    for ticker in nasdaq_tickers:
        if ticker in crash_results.keys():
            crash_plot = plot_crashes(nasdaq_data[ticker], crash_results[ticker], ticker)
            if crash_plot:
...
# MAGIC %md
# MAGIC ## 5. Debugging and addition of features
...
anomaly_data = {}
for i, (ticker, df) in enumerate(nasdaq_data.items()):
    anomalies = predict_anomalies(ticker, df, cont_val, ticker_count=i, total_ticker_no=len(nasdaq_data))
    if anomalies:
        if not anomalies.isEmpty():
            anomaly_data[ticker] = anomalies
...
# Determine why some stocks are failing to download, and whether they are the same each time
...
for i in range(5):
    ...
    if subset_tickers is not None:
        nasdaq_tickers = nasdaq_tickers[0:subset_tickers]
    nasdaq_data = download_stock_data(nasdaq_tickers, data_window, batch_size)
    print([ticker for ticker in nasdaq_tickers if ticker not in nasdaq_data.keys()])
    all_data[i] = nasdaq_data
...
[value.keys() for key, value in all_data.items()]

# COMMAND ----------

# Collate data for PowerBI

# def collate_crashes(stock_data, crash_data):
#     # Add tickers to the stock dfs and bind together
#     stock_dfs = []
#     for key, df in stock_data.items():
#         df["ticker"] = key  # Add the key as a new column
#         stock_dfs.append(df)
#     stock_df = pd.concat(stock_dfs, ignore_index=True)

#     # Add tickers to the crash dfs and bind together
#     crash_df = None
#     for key, df in crash_data.items():
#         df = df.withColumn("ticker", lit(key))  # Add the key as a new column
#         if crash_df is None:
#             crash_df = df
#         else:
...
#     collated_df = spark.createDataFrame(stock_df).join(crash_df, (stock_df["ticker"] == crash_df["ticker"]) & (stock_df \
...
# # Get categories of companies to filter for drug/device development

# # Create an empty list to store company information
# companies = []

# # Loop through tickers to get sector and industry information
...
# nasdaq_df = pd.DataFrame(companies)
...
# # Join crash and stock data together
# collated_df = stock_df.join(crash_df, (stock_df["ticker"] == crash_df["ticker"]) & (stock_df["date"] == crash_df["min_value_date"]), how="left")
# collated_df.columns
